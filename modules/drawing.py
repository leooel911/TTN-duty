...
from config import (
    TITLE, NATIONAL_HOLIDAYS, TRANSPORT_PERIODS, TAIWAN_TZ,
    C_HDR, C_EMPTY, C_WORK_BG, C_WEEKEND_BG, C_DO_BG, C_PAY_BG, C_TOWN_BG,
    C_DO_TXT, C_PAY_TXT, C_HOLI_TXT, C_OT_TXT, C_NOTE_TXT, C_TOWN_TXT
)
from modules.utils import is_town_shift, is_overtime
...
def build_weeks(start_dt, dates, cells):
    ...
    for dt, raw in zip(dates, cells):
        week.append((dt, parse_cell(raw), str(raw) if not str(raw) == 'nan' else ""))
        if len(week) == 7:
            weeks.append(week)
            week = []
    if week:
...
def render_schedule_figure(start_dt, dates, emp_id, emp_name, cells, unit_label, badge_title="Producer | C.L.F"):
        ...
        for ci, item in enumerate(week):
            ...
            if d.get("hours"): 
                ...
                if do_match and do_match != tr:
                    draw_bold_text(ax, x + CW - 0.026, ry + 0.026, do_match, ha="right", va="bottom", color=C_DO_TXT, fontproperties=fp(10.5))
    ...
    buf = io.BytesIO()
    plt.tight_layout(pad=0); plt.savefig(buf, format="png", dpi=300, bbox_inches="tight", facecolor="white", pad_inches=0.1); buf.seek(0)
    plt.close(fig)
    return buf
