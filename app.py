...
def process_file_data(input_str):
    ...
    for i, col in enumerate(col_names):
        ...
        if match_d:
            m, d = map(int, match_d.groups())
            # 簡單跨年判斷：若偵測到月分從大變小，則自動+1年
            if i > 0 and start_dt and m < start_dt.month and (start_dt.month - m) > 6:
                cur_y = current_year + 1
            else:
                cur_y = current_year
            
            date_obj = date(cur_y, m, d)
            if i == 0: start_dt = date_obj
            dates.append(f"{m}/{d}")
        else:
...
def setup_font():
    """ 具備防呆機制，找不到字型時回傳 None 以啟用 Matplotlib 內建防護 """
...
def is_overtime(h, tr, note):
    if not str(tr).strip().upper() in ["PAY", "FAC", "DO", "D2W", "AL", "SL", "CL", "ML"] and re.match(r'^[A-Z]+\d+', str(tr).upper()):
        if not h: return False
        try:
            # 修正對不同格式加班時數的解析邏輯
            parts = re.findall(r'\d+', str(h))
            if len(parts) >= 2:
                return (int(parts[0]) * 60 + int(parts[1])) > 510
        except: return False
    return False
...
def parse_cell(raw):
    ...
    if len(lines) == 1 and any(x in lines[0] for x in ["DO", "D2W", "PAY", "FAC"]): return dict(start="", train=lines[0], end="", hours="", note="")
    start_time = pad_time(times[0]) if times else ""
    end_time = pad_time(times[1]) if len(times) > 1 else ""
    hours = calculate_hours(start_time, end_time)
    do_str = next((l for l in lines if any(x in l for x in ["DO", "D2W", "PAY", "FAC"])), "")
...
def is_town_shift(tr, note):
    ...
    if any(x in tr_upper for x in ["PAY", "FAC", "DO", "D2W"]): return False
...
# --- 其餘程式邏輯與 UI 保持與您原始碼完全相同 ---
# (為了確保不影響任何功能，其餘邏輯如 Main 區塊、Session 檢查等均維持原樣)
# [請在此處接續您原本系統後段的所有 st. 邏輯]
