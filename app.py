...
if is_maintenance_mode():
    ...
    <div class="telemetry-card">
        ...
        <div class="telemetry-value" style="font-size: 22px; color: #60A5FA; margin-bottom: 8px;">{get_schedule_range()}</div>
        <div class="telemetry-sub">
            各職位更新時間：<br>
            - 駕駛：{get_file_mtime_str(ROLE_FILES['駕駛'])}<br>
            - 列車長：{get_file_mtime_str(ROLE_FILES['列車長'])}<br>
            - 服勤員：{get_file_mtime_str(ROLE_FILES['服勤員'])}
        </div>
    </div>
    """, unsafe_allow_html=True)
