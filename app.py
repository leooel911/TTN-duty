...
<style>
    ...
    .scroll-anchor { position: relative; top: -460px; visibility: hidden; }
...
if is_maintenance_mode() and not st.session_state["admin_bypassed"]:
    ...
    <div class="telemetry-card" style="border: 1px solid #EF4444; background: linear-gradient(135deg, #7F1D1D 0%, #450A0A 100%); max-width: 600px; margin: 4rem auto 2rem auto; text-align: center; padding: 30px;">
        ...
        <div class="telemetry-sub maint-sub" style="margin-top: 15px; font-size: 14px;">目前系統更新維護中，請稍後再試或洽系統管理員。</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if admin_login_clicked:
        if admin_bypass_input == ADMIN_PASSWORD: st.session_state["admin_bypassed"] = True; st.rerun()
        else: st.error("認證失敗：管理者密碼不符")
    st.stop()
...
if is_maintenance_mode() and st.session_state["admin_bypassed"]:
    ...
    <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid #EF4444; border-radius: 8px; padding: 10px 15px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center;">
        <span style="color: #FCA5A5; font-size: 13px; font-weight: 600;">⚠️ 【Admin Override 覆寫模式運作中】目前系統處於維護狀態，一般使用者存取已被隔離。</span>
    </div>
    """, unsafe_allow_html=True)
            ...
            if not any(os.path.exists(path) for path in ROLE_FILES.values()): st.error("無班表資料")
                ...
                try:
                    ...
                    for col_idx, bg_clr, border_clr, txt_clr, label in pill_legends:
                        ...
                        ax.add_patch(FancyBboxPatch((lx, legend_y), badge_w_leg, badge_h_leg, boxstyle="round,pad=0.002,rounding_size=0.008", linewidth=1.2, edgecolor=border_clr, facecolor=bg_clr))
                    ...
                    progress_bar.progress(100); status_placeholder.empty(); progress_bar.empty()
