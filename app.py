...
def process_file_data(file_source, target_id):
    ...
    for idx, row in df.iterrows():
        ...
        if found_id == target_clean:
            ...
            break
    if matched_row is None: raise ValueError(f"找不到員編「{target_id}」")
...
if st.button("立即生成個人班表圖片"):
    if not os.path.exists(SAVED_FILE_PATH): st.error("❌ 無資料")
        ...
        try:
            ...
            fig, ax = plt.subplots(figsize=(12, 8.5)) # 微調版面比例
            ...
            draw_bold_text(ax, ML + 0.008, ty + TH * 0.58, TITLE, ha="left", va="center", color="#FFFFFF", fontproperties=fp(14))
            draw_bold_text(ax, ML + 0.008, ty + TH * 0.25, f"CREW ID // {emp_id}    OPERATOR // {emp_name}    TIMELINE // {dates[0]} ~ {dates[-1]} ({len(dates)} DAYS)", ha="left", va="center", color="#CBD5E1", fontproperties=fp(10))
            ax.plot(0.97, ty + TH * 0.58, marker='o', markersize=4, color="#22C55E")
            draw_bold_text(ax, 0.975, ty + TH * 0.58, "C.L.F DESIGNS", ha="right", va="center", color="#FFFFFF", fontproperties=fp(9))
            ...
            for c in range(7):
                ...
                draw_bold_text(ax, x + CW / 2, dy + DH / 2, dlabels[c], ha="center", va="center", color="#000000", fontproperties=fp(10))

            # [省略部分迴圈內的繪圖邏輯，保持與原邏輯一致，僅調整 margins]
            # ... (繪圖內容維持原樣) ...
            
            # 底部資訊欄對齊
            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
            draw_bold_text(ax, ML, MB * 0.3, "DESIGNED BY: C.L.F // TECHNICAL SHIFT SYSTEM v4.19", ha="left", va="bottom", color="#64748B", fontproperties=fp(8))
            draw_bold_text(ax, 1.0 - MR, MB * 0.3, f"GENERATED: {now_str} | CONFIDENTIAL", ha="right", va="bottom", color="#64748B", fontproperties=fp(8))
            ...
            st.download_button("📥 下載班表", data=buf, file_name=f"TTN班表_{emp_name}.png", mime="image/png")
