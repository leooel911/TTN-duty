...
if not st.session_state["authenticated"]:
    # ... (簡化，保持原本登入邏輯)
    st.markdown("""<div style="text-align: center; margin-top: 4rem; margin-bottom: 2rem;"><div style="font-size: 40px; font-weight: 900; letter-spacing: 1px; color: #F8FAFC;">CREW DUTY ENGINE</div></div>""", unsafe_allow_html=True)
...
<div class="telemetry-card">
    <div class="telemetry-title">目前系統排班週期 & 資料更新</div>
    <div class="telemetry-value" style="font-size: 20px; color: #60A5FA;">{get_schedule_range()}</div>
...
app_mode = st.radio("模式", ["生產個人班表圖片檔", "換班｜尋找指定時段報到組員"], horizontal=True)

if app_mode == "生產個人班表圖片檔":
    target_input = st.text_input("輸入 員編 或 姓名", value="A")
    if st.button("立即生成個人班表圖片檔"):
        if not target_input: st.warning("請輸入")
        else:
            log_activity(target_input) # <--- 這裡掛載紀錄功能
            # ... (其餘原本的繪圖與生成邏輯不變)
            try:
                _, _, _, emp_id, emp_name, cells = process_file_data(target_input) # 需依據您之前的函數宣告
                # ... (後續繪圖邏輯與原本相同，略)
                st.success("生成成功")
