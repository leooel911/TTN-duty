...
if app_mode == "繪製個人月班表圖檔":
    ...
    if "ex_active_inspect_emp" not in st.session_state: st.session_state["ex_active_inspect_emp"] = None
        ...
        for idx, cand in enumerate(saved_candidates):
            ...
            with card_container:
                ...
                if st.button(btn_label, key=f"ex_gen_img_btn_{cand['員編']}_{idx}", use_container_width=True, type="secondary"):
                    if is_inspecting:
                        st.session_state["ex_active_inspect_emp"] = None
                    else:
                ...
                if is_inspecting:
                    status_placeholder = st.empty()
                    progress_bar = st.progress(0)
                    status_placeholder.markdown(f'<div class="loading-status-text">「{cand["姓名"]}」的完整月班表繪製中，請稍後...</div>', unsafe_allow_html=True)
                    progress_bar.progress(40)

                    try:
                        current_input = cand['員編']
                        ...
                        st.success(f"已成功載入 {emp_name} ({emp_id}) 之完整月班表")
                        render_zoomable_image(buf)
                        st.download_button("下載此組員月班表圖檔", data=buf, file_name=f"{current_unit_label}_班表_{emp_name}.png", mime="image/png", key=f"ex_dl_btn_{cand['員編']}_{idx}")
                    except Exception as e:
                        status_placeholder.empty()
                        progress_bar.empty()
                        st.error(f"載入完整班表時發生錯誤: {e}")
