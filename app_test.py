...
if st.session_state.get("inspect_emp_target") is not None:
    ...
    try:
        ...
        draw_bold_text(ax, ML, MB * 0.12, "DESIGNED BY: C.L.F // v4.19 (Sandbox)", ha="left", va="bottom", color="#0F172A", fontproperties=fp(12))
...
if st.session_state.get("nav_mode") == "admin_panel" and st.session_state.get("admin_logged_in", False):
    ...
    <div class="section-header-box">
        ...
        <div class="section-subtitle">Direct Administrator Dashboard // Shift Exchange & Base Engine</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.success("歡迎回來，管理員 LEO（測試環境控制台）")
    ...
    st.markdown("---")
    st.subheader("管理員檔案上傳區（支援 20號基準表與 21號後異動檔）")
    selected_role = st.selectbox("選擇要管理或上傳的職位類別", ["駕駛", "列車長", "服勤員"])
    ...
    with col_up1:
        ...
        uploaded_file = st.file_uploader(f"上傳最新班表 ({selected_role})", type=["xlsx", "xls", "csv"], key=f"up_{selected_role}")
    ...
    with col_up2:
        ...
        if base_file is not None:
            with open(base_target_path, "wb") as f:
...
<div class="{card_class}">
    <div class="telemetry-title">目前系統排班週期 & 伺服器資料狀態 (Sandbox)</div>
...
# (以下各模組與原本正式版保持完全一致，會自動套用前面升級過的 parse_file_data 與時間字典對照邏輯)
if app_mode == "繪製個人月班表圖檔":
    ...
    """, unsafe_allow_html=True)
    # (省略部分與原版相同之快篩渲染邏輯以確保簡潔，可直接延續原有渲染)
...
footer_badge_label = "ADMIN PANEL // C.L.F EDITION (SANDBOX)" if st.session_state.get("admin_logged_in", False) else "C.L.F EDITION (SANDBOX)"
